...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i node_modules | findstr /v /i '.basename(__FILE__).' | findstr /r /i /M /F:/ ":config_key :extension_class :full_namespace :full_namespace_double_backslash :package_description :package_name :plugin_class :plugin_class_lowercase :plugin_name :plugin_name_slug :webpack_asset_name :vendor_name_slug"'));
}

function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('grep -E -r -l -i ":config_key|:extension_class|:full_namespace|:full_namespace_double_backslash|:package_description|:package_name|:plugin_class|:plugin_class_lowercase|:plugin_name|:plugin_name_slug|:webpack_asset_name|:vendor_name_slug" --exclude-dir=vendor --exclude-dir=node_modules --exclude-dir=tests/Application/vendor --exclude-dir=tests/Application/node_modules ./* ./.github/* | grep -v '.basename(__FILE__)));
}
